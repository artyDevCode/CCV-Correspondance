...
{
    public class HomeSubjectController : Controller
    {

        CorrespondanceDB _db;

        public HomeSubjectController()
        {
            _db = new CorrespondanceDB();

        }

        public HomeSubjectController(CorrespondanceDB db)
        {
            _db = db;
        }
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            {
                List<CorrespondanceSubjectVM> aa = new List<CorrespondanceSubjectVM>();
                foreach (var group in model)
                {
                    var test = group.VM_Subject.Replace(" ", "").Trim('-').ToLower();
                    if (test.Equals(param.first_data) &&
                        group.VM_Type.ToLower().Contains(param.second_data.Trim()))
                        aa.Add(group);
                }

                modelsorted = aa.OrderBy(r => r.VM_Subject).ThenBy(e => e.VM_Type).ToList();
                aa = null;
                aaData = modelsorted.Select(d => new string[] { d.VM_Subject, d.VM_Type, d.VM_Year.ToString(), d.VM_FromTo, d.VM_Department, (d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : d.VM_DateOnLetter.ToString()), d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            if (param.sSearch != null)
            {
                if (param.sSearch.Length > 2)
                {
                    ...
                    modelsorted = aa.OrderBy(r => r.VM_Subject).ThenBy(e => e.VM_Type).ThenBy(e => e.VM_Year).ToList();
                    aa = null;
                    aaData = modelsorted.Select(d => new string[] { d.VM_Subject, d.VM_Type, d.VM_Year.ToString(), d.VM_FromTo, d.VM_Department, (d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : d.VM_DateOnLetter.ToString()), d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            var vv = model.Select(r => new { r.VM_Subject, r.VM_Type }).Distinct().OrderBy(r => r.VM_Subject).ThenBy(e => e.VM_Type).ToList();

            aaData = vv.Select(d => new string[] { d.VM_Subject, d.VM_Type, "", "", "", "", "", "" }).ToArray();
