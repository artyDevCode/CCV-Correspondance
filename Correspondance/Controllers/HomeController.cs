...
namespace CCVCorrespondance.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(CorrespondanceDB db)
        {
            _db = db;
        }
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            {
                List<CorrespondanceVM> aa = new List<CorrespondanceVM>();
                foreach (var group in model)
                {
                    var test = group.VM_Type.Replace(" ", "").Trim('-').ToLower();
                    if (test.Contains(param.first_data) &&
                        group.VM_Year.ToString().Contains(param.second_data.Trim()))
                        aa.Add(group);
                }

                modelsorted = aa.OrderBy(r => r.VM_Type).ThenBy(e => e.VM_Year).ToList();
                aa = null;
                //RIC TERNARY 
                aaData = modelsorted.Select(d => new string[] { d.VM_Type, d.VM_Year.ToString(), d.VM_FromTo, d.VM_Department, d.VM_Subject, (d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : d.VM_DateOnLetter.ToString()), d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            var vv = model.Select(r => new { r.VM_Type, r.VM_Year }).Distinct().OrderBy(r => r.VM_Type).ThenBy(e => e.VM_Year).ToList();

            aaData = vv.Select(d => new string[] { d.VM_Type, d.VM_Year.ToString(), "", "", "", "", "", "" }).ToArray();
