...
{
    public class HomeFromController : Controller
    {

        CorrespondanceDB _db;

        public HomeFromController()
        {
            _db = new CorrespondanceDB();

        }

        public HomeFromController(CorrespondanceDB db)
        {
            _db = db;
        }
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            var model = _db.Correspondances
               ...
               .Select(r => new CorrespondanceFromToVM { VM_FromTo = r.CorrespondanceFromTo, VM_Type = r.CorrespondanceType, VM_Year = r.CorrespondanceYear, VM_Department = r.CorrespondanceDepartment, VM_Subject = r.CorrespondanceSubject, VM_DateOnLetter = r.CorrespondanceDateOnLetter, VM_DateReceivedOrSent = r.CorrespondanceDateReceivedOrSent, VM_Id = r.ID })
                .ToList();

            List<CorrespondanceFromToVM> modelsorted;
            ...
            {
                List<CorrespondanceFromToVM> aa = new List<CorrespondanceFromToVM>();
                foreach (var group in model)
                {
                    var test = group.VM_FromTo.Replace(" ", "").Trim('-').ToLower();
                    var test1 = test.Replace(",", "").Trim('-').ToLower();
                    if (test1.Contains(param.first_data) &&
                        group.VM_Type.ToLower().Contains(param.second_data.Trim()))
                        aa.Add(group);
                }

                modelsorted = aa.OrderBy(r => r.VM_FromTo).ThenBy(e => e.VM_Type).ToList();
                aa = null;
                aaData = modelsorted.Select(d => new string[] { d.VM_FromTo, d.VM_Type, d.VM_Year.ToString(), d.VM_Department, d.VM_Subject, (d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : d.VM_DateOnLetter.ToString()), d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            if (param.sSearch != null)
            {
                if (param.sSearch.Length > 2)
                {
                    ...
                    modelsorted = aa.OrderBy(r => r.VM_FromTo).ThenBy(e => e.VM_Type).ThenBy(e => e.VM_Year).ToList();
                    aa = null;
                    aaData = modelsorted.Select(d => new string[] { d.VM_FromTo, d.VM_Type, d.VM_Year.ToString(), d.VM_Department, d.VM_Subject, (d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : d.VM_DateOnLetter.ToString()), d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            var vv = model.Select(r => new { r.VM_FromTo, r.VM_Type }).Distinct().OrderBy(r => r.VM_FromTo).ThenBy(e => e.VM_Type).ToList();

            aaData = vv.Select(d => new string[] { d.VM_FromTo, d.VM_Type, "", "", "", "", "", "" }).ToArray();
