...
namespace CCVCorrespondance.Controllers
{
    public class HomeDateController : Controller
    {
        ...
        public HomeDateController()
        {
            _db = new CorrespondanceDB();

        }

        public HomeDateController(CorrespondanceDB db)
        {
            _db = db;
        }
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            var model = _db.Correspondances
               ...
               .Select(r => new CorrespondanceDateVM { VM_Subject = r.CorrespondanceSubject, VM_Type = r.CorrespondanceType, VM_Year = r.CorrespondanceYear, VM_FromTo = r.CorrespondanceFromTo, VM_Department = r.CorrespondanceDepartment, VM_DateOnLetter = r.CorrespondanceDateOnLetter, VM_DateReceivedOrSent = r.CorrespondanceDateReceivedOrSent, VM_Id = r.ID })
                .ToList();

            List<CorrespondanceDateVM> modelsorted;
            ...
            {
                List<CorrespondanceDateVM> aa = new List<CorrespondanceDateVM>();
                foreach (var group in model)
                {

                    //sdate = group.VM_DateOnLetter.ToString();
                    //date = DateTime.ParseExact(sdate, @"dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture);

                    //var test = group.VM_DateOnLetter.ToString().Replace(" ", "").Trim('-').ToLower();

                    var test = group.VM_DateOnLetter.HasValue ? group.VM_DateOnLetter.Value.ToString("ddMMyyyy").Replace(" ", "").Trim('-').ToLower() : "NoDate";
                    if (test.Contains(param.first_data) &&
                        group.VM_Type.ToLower().Contains(param.second_data.Trim()))
                        aa.Add(group);
                }

                modelsorted = aa.OrderBy(r => r.VM_DateOnLetter).ThenBy(e => e.VM_Type).ToList();
                aa = null;
                aaData = modelsorted.Select(d => new string[] { d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : "NoDate", d.VM_Type, d.VM_Year.ToString(), d.VM_FromTo, d.VM_Department, d.VM_Subject, d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            if (param.sSearch != null)
            {
                if (param.sSearch.Length > 2)
                {
                    ...
                    modelsorted = aa.OrderByDescending(r => r.VM_DateOnLetter).ThenBy(e => e.VM_Type).ThenBy(e => e.VM_Year).ToList();
                    aa = null;
                    aaData = modelsorted.Select(d => new string[] { d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : "NoDate" , d.VM_Type, d.VM_Year.ToString(), d.VM_FromTo, d.VM_Department, d.VM_Subject, d.VM_DateReceivedOrSent.ToString("dd-MM-yyyy"), d.VM_Id.ToString() }).ToArray();
            ...
            var vv = model.Select(r => new { r.VM_DateOnLetter, r.VM_Type }).Distinct().OrderBy(r => r.VM_DateOnLetter).ThenBy(e => e.VM_Type).ToList();

            aaData = vv.Select(d => new string[] {  d.VM_DateOnLetter.HasValue ? d.VM_DateOnLetter.Value.ToString("dd-MM-yyyy") : "NoDate" , d.VM_Type, "", "", "", "", "", "" }).ToArray();
